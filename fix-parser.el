...
(defvar fix-parser/fix-tag-dictionary-4.4
  (let ((dict (make-hash-table :test 'equal)))
    ...
    ;; User-defined and custom tags often start in the 5000-9999 range The user provided this example,
...
parses each one.  The message is expected to use '|' as the field separator."
  ...
  (let ((output-buffer-name "*fix-parse-result*")
        (tables '()))
    ;; Determine which text to parse: region or whole buffer
    (if (use-region-p)
      ...
      ;; --- Case 2: No region, scan whole buffer ---
      (let* (;; This is the compatible way to split messages.
        ...
        (dolist (msg potential-messages)
          ;; The helper function validates each potential message.
          ;; Garbage created by text preceding the first "8=FIX" will be ignored.
